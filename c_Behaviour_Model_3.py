# -*- coding: utf-8 -*-
"""
Created on Thu Apr 15 14:46:39 2021
...
import constants
from constants import current_folder_path, chart_config
from stage_three_read_data import return_excel_data
from final_chart import run_charts

'''
runs from test case CFD output folder when script run only without gui
'''
# ## 1. Functions
...
from escape_probability import prob_chart_return_data
def run_excel_sheets(cfd_output_path):
    ...
    # replace below with better method - from appendix 
    ...
    while n < len(Scenario_Names): # for each scenario in the list of the scenario names
        ...
        # TODO: do not redo for all scenarios
        Pre_dict = prob_chart_return_data(results_path=cfd_output_path, scenario=Scenario_Names[n], model_name='')
        workbook = openpyxl.load_workbook(f"{cfd_output_path}/{Scenario_Names[n]}/{Scenario_Names[n]}_Results.xlsx") # creates spreadsheet for results
        ...
        results_file = f'{Scenario_Names[n]}_Results.xlsx'
        workbook.save(f'{results_dir}\{results_file}')
        workbook.close()

        '''
        
        Decision Tree to be created in template report
        cells to be updated both in spreadsheet and report
...
# TODO: use is multistorey, is > 12x16; kitchen open with flat > 8 x 12 as parameters
...
def run_stage_three(
    ...
    run_excel_sheets(cfd_output_path) 
    # run numbers through report
...
if __name__ == '__main__':
    ...
    run_stage_three(cfd_output_path=r'CFD Test Output\Roneo Corner - Smallest Flat')
