...
def generate_data(
    ...
    for count in range(max(1, int(No_Runs / sum(Pre_dict.values())))): # if pre_dict is same length or longer, then uses dict 1x
    ...
    while n < No_Runs:   ## main loop for model
        ...
        random_vis_ten.append(VisTen)
        loc_dat = TD_List.pop()  ## picks a random starting location and TD
        location = loc_dat[0]   ## writes to spreadsheet
        # worksheet[f"C{n+2}"]=location # just use location
        random_location.append(location)
        ...
        if Harmed == "yes":
            ...
            # worksheet[f"I{n+2}"]="yes" # occupant harmed
            is_harmed.append("yes")
        else:
            # worksheet[f"I{n+2}"]="no"
            is_harmed.append("no")        
        if Escaped == "no":
            number_trapped = number_trapped + 1
            # worksheet[f"H{n+2}"]="yes" # occupant trapped
            is_trapped.append("yes")

            # worksheet[f"F{n+2}"]="N/A" # escape ends
            escape_ends.append("N/A")

            # worksheet[f"G{n+2}"]="N/A" # escape time
            escape_times.append("N/A")
        else:
            # worksheet[f"H{n+2}"]="no"
            is_trapped.append("no")   
            # worksheet[f"F{n+2}"]=time-1
            escape_ends.append(time-1)
            # worksheet[f"G{n+2}"]=time-1-pre
            escape_times.append(time-1-pre) # now considers pre-evacuation time
        ...
        min_visibility.append(min_vis)
        # worksheet[f"K{n+2}"]=max_temp
        max_temp_list.append(max_temp)
        # worksheet[f"M{n+2}"]=FED_toxc
        FED_toxc_list.append(FED_toxc)
        # worksheet[f"L{n+2}"]=max_rad
        max_radiation.append(max_rad)
        # worksheet[f"N{n+2}"]=FED_heat
        FED_heat_list.append(FED_heat)        
    ...
    while n <= Simulation_Time:
        # worksheet[f"P{n+2}"]=n
        sim_time_list.append(n) # column P
        current_timestep = n
        # worksheet[f"Q{n+2}"]=f"""=(COUNTIF(F$2:F${No_Runs+1}, "<="&P{n+2}))/{No_Runs}*100"""
        # column F escape ends
        # P - time
        column_Q_count = sum(1 for value in escape_ends if type(value) != str and value <= current_timestep) # escape ends
        escape_ends_total.append(column_Q_count)
        # worksheet[f"R{n+2}"]=f"""=(COUNTIF(G$2:G${No_Runs+1}, "<="&P{n+2}))/{No_Runs}*100"""
        # column G escape times
        column_R_count = sum(1 for value in escape_times if type(value) != str and value <= current_timestep) # escape times
        escape_times_total.append(column_R_count)
    ...
    if number_trapped == 0:
        ...
        trapped_fraction = number_trapped/No_Runs    
    ...
    if number_harmed == 0:
        ...
        harmed_fraction = number_harmed/No_Runs
    ...
    if No_Runs-number_trapped == 0:
        ...
        escape_fraction = (No_Runs-number_trapped)/No_Runs
    ...
    # while n < len(HRR):
    #     # why are n and n+1 filled already?
    #     # worksheet[f"AF{n+2}"]=HRR_Time[n]
    #         # use HRR time!!    
    #     # worksheet[f"AG{n+2}"]=HRR[n]
    #     # use HRR!!
    #     # worksheet[f"AH{n+2}"]=HRR[n+1]
    ...
    print(f"{No_Runs}: {sn} - {name}: done")
    print(escape_fraction, harmed_fraction, trapped_fraction)
    return escape_fraction, harmed_fraction, trapped_fraction
    # return load of stuff

if __name__ == '__main__':
    import openpyxl
    from escape_probability import return_dataset_dict_from_curve
    No_Runs = 100
    Pre_dict = return_dataset_dict_from_curve(number_of_runs=No_Runs)
